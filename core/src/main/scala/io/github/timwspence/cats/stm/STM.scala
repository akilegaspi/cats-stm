...
sealed abstract class STM[+A] {
  ...
  /**
    ...
    */
  final def orElse(fallback: STM[A]): STM[A] = OrElse(this, fallback)
...
object STM {
  ...
  final class AtomicallyPartiallyApplied[F[_]] {
    ...
    private def run[A](stm: STM[A]): TResult[A] = {
      var conts: List[Any => STM[Any]] = Nil
      var state: Map[TVar, Int] = Map.empty

      @tailrec
      def go(stm: STM[Any]): TResult[Any] = stm match {
        case Pure(a) => {
          if (conts.isEmpty) TSuccess(a) else {
            val f = conts.head
            conts = conts.tail
            go(f(a))
          }
        ...
        case Bind(stm, f) =>
          conts = f :: conts
          go(stm)
        case Get(tvar) => ??? //TODO update state
        case Modify(tvar, f) => ??? //TODO update state
        case OrElse(attempt, fallback) => ??? //TODO possibly need another set of stacks to handle this?
        case Abort(erro) => TFailure(error)
        case Retry => TRetry
...
private[stm] final case class Bind[+A, B](stm: STM[B], f: B => STM[A]) extends STM[A]
private[stm] final case class Get[+A](tvar: TVar[A]) extends STM[A]
private[stm] final case class Modify[+A](tvar: TVar[A], f: A => A) extends STM[Unit]
private[stm] final case class OrElse[+A](attempt: STM[A], fallback: STM[A]) extends STM[A]
private[stm] final case class Abort(error: Throwable) extends STM[Nothing]
private[stm] final case object Retry extends STM[Nothing]
